...
work items (e.g. move a key from `excluded` to `inventory` with `support: unknown` and open an
issue).
...
- Found in `excluded` → **Dismissed.** Tell the user the key is in `excluded`. Ask whether to
  proceed with a fresh analysis anyway. If not, stop.
- Found in `inventory` → **Known.** Note the current entry and its `support` tag but do not let it
  influence the analysis in Step 3. The goal is a clean-room result to compare against.
...
The sub-agent returns a `ProposedSupport` for each key plus support-specific fields (`Severity`,
`Description`, `Discussion`). These map directly to inventory `support` tags — no translation
needed. Treat the sub-agent's `ProposedSupport` as a proposal, not a final decision:

- `excluded` always requires explicit user confirmation before writing.
- `saluki_keys.rs` means the key has no Agent counterpart; flag it to the user before touching
  `saluki_keys.rs`.
- `unknown` is appropriate when the sub-agent could not determine scope or implementation status;
  confirm with the user before accepting.
...
If the analysis confirms out-of-scope, recommend leaving it in `excluded`. If the analysis finds it
is relevant after all, propose moving it to `inventory` with `support: unknown` as the default
holding pen. Ask the user to confirm.

## Step 5: Update Overlay

Apply the confirmed changes to `schema_overlay.yaml`. Follow `/config-management` rules:

- Section order: `inventory`, `excluded`

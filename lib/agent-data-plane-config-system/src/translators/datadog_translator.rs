...
impl DatadogConfigWitness for DatadogTranslator<'_> {
    ...
    fn consume_forwarder_retry_queue_max_size(&mut self, value: Option<i64>) {
        self.config.shared.endpoints.forwarder.retry_queue_max_size =
            value.and_then(|v| drop_when_schema_default(v.max(0) as u64, DEFAULT_FORWARDER_RETRY_QUEUE_MAX_SIZE));
    }

    fn consume_forwarder_retry_queue_payloads_max_size(&mut self, value: Option<i64>) {
        self.config.shared.endpoints.forwarder.retry_queue_payloads_max_size = value
            .and_then(|v| drop_when_schema_default(v.max(0) as u64, DEFAULT_FORWARDER_RETRY_QUEUE_PAYLOADS_MAX_SIZE));
    }

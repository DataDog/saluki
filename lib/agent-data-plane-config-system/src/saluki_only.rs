//! [`SalukiOnly`]: the ADP-only slice of the source configuration, plus [`seed`].
//!
//! # What this is
//!
...
pub struct SalukiOnly {
    ...
    /// Idle counter keep-alive, in seconds (`counter_expiry_seconds`, legacy `dogstatsd_expiry_seconds`).
    #[serde(alias = "dogstatsd_expiry_seconds")]
...
impl SalukiOnly {
    ...
    pub(crate) fn seed(&self, config: &mut SalukiConfiguration) {
        ...
        if let Some(v) = self.counter_expiry_seconds {
            dsd.aggregation.counter_expiry_seconds = Some(v);
        }
...
mod tests {
    ...
    fn every_saluki_only_key_transports_to_the_model() {
        ...
        assert_eq!(dsd.aggregation.counter_expiry_seconds, Some(120));

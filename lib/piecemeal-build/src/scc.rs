...
fn scc(
    vertices: &[MessageIndex], desc: &FileDescriptor, u: usize, count: &mut isize, low: &mut [isize],
    disc: &mut [isize], stack: &mut Vec<usize>, sccs: &mut Vec<Vec<usize>>, ids: &HashMap<MessageIndex, usize>,
) {
    disc[u] = *count;
    low[u] = *count;
    *count += 1;
...
impl FileDescriptor {
    fn flatten_messages(&self) -> Vec<MessageIndex> {
        let mut all_msgs = self.messages.iter().map(|m| m.index.clone()).collect::<Vec<_>>();
    ...
    pub fn sccs(&self) -> Vec<Vec<MessageIndex>> {
        ...
        for u in 0..vertices.len() {
            if disc[u] == -1 {
                scc(
                    &vertices, self, u, &mut count, &mut low, &mut disc, &mut stack, &mut sccs, &ids,
                );
            }
        }
        sccs.into_iter()

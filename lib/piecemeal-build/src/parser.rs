...
use crate::types::{
    Enumerator, Extend, Extensions, Field, FieldType, FileDescriptor, Frequency, Message, OneOf, Proto2Frequency,
...
use nom::{
    ...
    character::complete::{alpha1, alphanumeric1, anychar, digit1, hex_digit1, multispace1, not_line_ending},
...
fn hex_integer(input: &str) -> IResult<&str, i32> {
    preceded(tag("0x"), map_res(hex_digit1, |s: &str| i32::from_str_radix(s, 16)))(input)
}
...
fn default_check<'a>(
    syntax: Syntax, typ: FieldType, key_vals: &[(&'a str, &'a str)],
...
mod test {
    ...
    fn test_word_and_qualifiable_names() {
        ...
        fn assert_both_ok_different_res(str_to_parse: &str, word_res: &str, qualifiable_name_res: &str) {

...
impl Frequency {
    pub fn is_map(&self) -> bool {
        matches!(
            self,
            Frequency::Proto2Frequency(Proto2Frequency::Map) | Frequency::Proto3Frequency(Proto3Frequency::Map)
        )
    }
...
impl FieldType {
    ...
    fn has_lifetime(
        &self, desc: &FileDescriptor, config: &Config, packed: bool, ignore: &mut Vec<MessageIndex>,
    ) -> bool {
        match *self {
...
impl Field {
    ...
    fn write_custom_default<W: Write>(&self, w: &mut W, desc: &FileDescriptor, config: &Config) -> Result<()> {
    ...
    fn write_default<W: Write>(&self, w: &mut W, desc: &FileDescriptor, config: &Config) -> Result<()> {
            ...
            GeneratedType::ArrayLikeType => {
                if self.packed() {
                    if self.typ.is_fixed_size() {
                        ...
                        writeln!(w, "msg.{} = r.read_packed(bytes, |r, bytes| Ok({}))?,", name, val_cow)?;
                    }
                } else {
...
impl Message {
    fn write_default_consts<W: Write>(&self, w: &mut W, desc: &FileDescriptor, config: &Config) -> Result<()> {
    ...
    fn has_lifetime(&self, desc: &FileDescriptor, config: &Config, ignore: &mut Vec<MessageIndex>) -> bool {
        ...
        let res = self.all_fields().any(|f| {
            f.typ.has_lifetime(desc, config, f.packed(), ignore) && (!f.deprecated || config.add_deprecated_fields)
        });
        ignore.pop();
        res
    }
    ...
    fn is_unit(&self) -> bool {
        self.fields.is_empty() && self.oneofs.is_empty() && self.messages.iter().all(|m| m.is_unit())
    }

    fn write_common_uses<W: Write>(
        w: &mut W, messages: &[Message], desc: &FileDescriptor, config: &Config,
    ) -> Result<()> {
        if config.nostd {
        ...
        if config.nostd
            && messages
                .iter()
                .any(|m| desc.owned && m.has_lifetime(desc, config, &mut Vec::new()) || m.all_fields().any(|f| f.boxed))
        {
            writeln!(w)?;
    ...
    fn write_impl_message_info<W: Write>(&self, w: &mut W, desc: &FileDescriptor, config: &Config) -> Result<()> {
        let mut ignore = Vec::new();
        if config.dont_use_cow {
            ignore.push(self.index.clone());
        }
    ...
    fn write_message_builder_field<W: Write>(
        ...
        match field.typ.category() {
            ...
            FieldCategory::Message => self.write_message_builder_field_message(w, field, desc, config),
    ...
    fn write_message_builder_field_map<W: Write>(&self, w: &mut W, field: &Field, desc: &FileDescriptor) -> Result<()> {
        ...
        if key_field_type.category() != FieldCategory::Scalar || value_field_type.category() != FieldCategory::Scalar {
...
impl FileDescriptor {
                    ...
                    if is_cycle(cycle, self) {
                        if error_cycle {
                            return Err(Error::Cycle(
                                cycle.iter().map(|m| m.get_message(self).name.clone()).collect(),
                            ));
                        } else {
                            for v in cycle {
                                warn!(
                                    ...
                                    cycle.iter().map(|m| &m.get_message(self).name).collect::<Vec<_>>()
                                );
                                for f in v
                                    .get_message_mut(self)
                                    .all_fields_mut()
                                    .filter(|f| !(f.frequency.is_optional() || f.frequency.is_repeated()))
                                    .filter(|f| f.typ.message().map_or(false, |m| cycle.contains(m)))
                                {
                                    f.boxed = true;
    ...
    fn get_full_names(&mut self) -> (HashMap<String, MessageIndex>, HashMap<String, EnumIndex>) {
        fn rec_full_names(
            m: &mut Message, index: &mut MessageIndex, full_msgs: &mut HashMap<String, MessageIndex>,
    ...
    fn resolve_types(&mut self) -> Result<()> {
        ...
        fn rec_resolve_types(
            m: &mut Message, full_msgs: &HashMap<String, MessageIndex>, full_enums: &HashMap<String, EnumIndex>,
        ) -> Result<()> {

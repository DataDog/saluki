//! Component-native configuration for the Datadog service checks encoder.
//!
//! Mirrors `DatadogServiceChecksConfiguration` in `saluki-components` with source key names and the
//! `Deserialize` impl stripped.

/// Configuration for the Datadog service checks encoder component.
///
/// Mirrors `DatadogServiceChecksConfiguration` in `saluki-components`.
#[derive(Clone, Debug, PartialEq, serde::Serialize)]
pub struct DatadogServiceChecksConfig {
...
impl Default for DatadogServiceChecksConfig {
    fn default() -> Self {
        Self {

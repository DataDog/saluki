...
impl InternerStorage {
    ...
    fn add_reclaimed(&mut self, entry: ReclaimedEntry) {
        ...
        // it, since we can just wind back `len` to reclaim the space.
        let merged_entry = self.reclaimed.insert(entry);
        if merged_entry.offset() + merged_entry.capacity() == self.len {
            self.len -= merged_entry.capacity();
...
mod tests {
    ...
    fn has_reclaimed_entries_string_fits_exactly() {
        ...
        let s3 = interner.try_intern("hello world~");
        assert_eq!(s3, None);
        ...
        let _s3 = interner.try_intern("hello world~").expect("should not fail to intern");

...
/// A borrowed representation of an origin.
///
...
pub struct OriginRef<'a> {
    ...
    external_data: Option<ExternalDataRef<'a>>,
}
...
impl<'a> OriginRef<'a> {
    ...
    pub fn set_external_data(&mut self, external_data: impl Into<Option<&'a str>>) {
        self.external_data = external_data.into().and_then(ExternalDataRef::from_raw);
    }

    /// Returns the external data of the sender.
    pub fn external_data(&self) -> Option<&ExternalDataRef<'a>> {
        self.external_data.as_ref()
    }
}

impl<'a> fmt::Display for OriginRef<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
///
/// For parsing external data strings without allocating, see [`ExternalDataRef`].
...
impl<'a> fmt::Display for ExternalDataRef<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "pu-{},cn-{},it-{}",

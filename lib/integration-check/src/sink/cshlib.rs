...
pub mod callback {
    ...
    pub type SubmitEventPlatformEvent =
        unsafe extern "C" fn(id: *const c_char, event: *const c_char, event_len: c_int, event_type: *const c_char);
...
fn run_check<C>(callback: &callback::Callback, check_id: &str, init_config: &str, instance_config: &str) -> Result<()>
where
    C: Check<Snk = SharedLibrary>,
{
    let init_config: serde_yaml::Mapping =
        serde_yaml::from_str(init_config).with_context(|| "Failed to parse init configuration")?;
    let instance_config: serde_yaml::Mapping =
        serde_yaml::from_str(instance_config).with_context(|| "Failed to parse instance configuration")?;
...
fn map_to_cstring_array(map: &HashMap<String, String>) -> Vec<CString> {
    map.iter().map(|(k, v)| to_cstring(format!("{}:{}", k, v))).collect()
}
...
fn run_impl<C>(
    callback: *const callback::Callback, check_id: *const c_char, init_config: *const c_char,
    ...
    C: Check<Snk = SharedLibrary>,
{
    ...
    let instance_config = str_from_ptr(instance_config).ok_or(anyhow!("invalid instance_config"))?;

    run_check::<C>(callback, check_id, init_config, instance_config)
}

pub fn run<C>(
    check_id: *const c_char, init_config: *const c_char, instance_config: *const c_char,
    callback: *const callback::Callback, error: *mut *const c_char,
) where
    C: Check<Snk = SharedLibrary>,
{

...
#[cfg(all(not(windows), feature = "cng-fips"))]
compile_error!("The `cng-fips` feature is only supported on Windows builds; use `fips` for non-Windows FIPS builds.");
#[cfg(all(unix, not(any(feature = "fips", feature = "cng-fips"))))]
use std::os::unix::fs::OpenOptionsExt;
#[cfg(not(any(feature = "fips", feature = "cng-fips")))]
use std::{
    fmt::{Debug, Formatter},
    fs::{File, OpenOptions},
    io::{self, Write},
...
#[cfg(any(feature = "fips", feature = "cng-fips"))]

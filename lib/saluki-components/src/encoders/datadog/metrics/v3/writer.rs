...
use super::telemetry::V3ValueStats;
...
pub(crate) struct V3EncoderStats {
    pub(crate) value_stats: V3ValueStats,
    pub(crate) columns: Vec<V3ColumnBytes>,
}

/// Serialized bytes for a single V3 column.
...
impl<'a> V3MetricBuilder<'a> {
    ...
    fn compact_values(&mut self) {
        ...
        if is_sketch {
            ...
            self.writer.value_stats.sint64 += count as u64;
        }
        ...
        match val_ty {
            V3ValueType::Zero => {
                self.writer.value_stats.zero += float_values_len as u64;
                // Values are all zero, don't store anything
                self.writer.vals_float64.truncate(start);
            }
            V3ValueType::Sint64 => {
                self.writer.value_stats.sint64 += float_values_len as u64;
                if is_sketch {
            ...
            V3ValueType::Float32 => {
                self.writer.value_stats.float32 += float_values_len as u64;
                for i in start..end {
            ...
            V3ValueType::Float64 => {
                self.writer.value_stats.float64 += float_values_len as u64;
...
fn write_column<F>(
    output: &mut Vec<u8>, columns: &mut Vec<V3ColumnBytes>, field_number: u32, write: F,
) -> Result<(), GenericError>
where
    F: FnOnce(&mut CodedOutputStream<'_>) -> protobuf::Result<()>,
{
    ...
    if !bytes.is_empty() {
        columns.push(V3ColumnBytes { field_number, bytes });
    }
...
mod tests {
    ...
    fn test_serialize_basic_metric() {
        ...
        assert!(!encoded.payload.is_empty());
        assert_eq!(encoded.stats.value_stats.sint64, 1);
    }
}

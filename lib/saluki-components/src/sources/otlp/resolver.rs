...
///
/// The returned worker drives cache expiration and interner telemetry for the resolver. It must be spawned under the
/// component's supervisor -- see `Otlp::run` -- or none of that work happens.
///
/// # Errors
///
...
pub fn build_context_resolver(
...
) -> Result<(ContextResolver, CompositeWorker), GenericError> {
    ...
    let (tags_resolver, tags_resolver_worker) =
        TagsResolverBuilder::new(format!("{}/otlp/tags", context.component_id()), interner.clone())?
    ...
    let (resolver, resolver_worker) =
        ContextResolverBuilder::from_name(format!("{}/otlp/primary", context.component_id()))?
    ...
    // One child for both: they are restarted together, and a panic in either brings the group back.
    let worker = CompositeWorker::new(
        "context_resolver",
        vec![Box::new(tags_resolver_worker), Box::new(resolver_worker)],
    );

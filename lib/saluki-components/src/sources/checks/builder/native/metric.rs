...
use dd_rs_checks::metric::{Metric as RsMetric, Type};
use dd_rs_checks::sink::{event, histogram, service_check};
...
pub fn service_check_to_event(
    check: service_check::ServiceCheck,
...
pub fn histogram_to_event(
    histogram: histogram::Histrogram,
    execution_context: &ExecutionContext,
) -> Event {
    let mut tags = TagSet::default();
    histogram

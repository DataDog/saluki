...
impl MemoryBounds for ChecksConfiguration {
    fn specify_bounds(&self, builder: &mut MemoryBoundsBuilder) {
        ...
        builder.minimum().with_fixed_amount("CheckDispatcher", 0);
    }
}
            ...
            loop {
                select! {
                    Some(check_request) = check_run_requests.recv() => {
                        info!("Processing check request: {}", check_request);
                        ...
                        if python_scheduler.can_run_check(&check_request) {
                            ...
                            match python_scheduler.run_check(&check_request) {
                                ...
                                Err(e) => {
                                    error!("Error dispatching check request (will continue): {}", e);
                                    // Continue processing other checks instead of letting the error propagate
                                }
                            }
                        } else {
                            warn!("Check request not dispatched (no matching python executor): {}", check_request);
                        }
                    },
                    Some(check_request) = check_stop_requests.recv() => {
                        ...
                        match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                            ...
                            Ok(_) => debug!("Successfully stopped check"),
                            Err(e) => error!("Error stopping check: {:?}", e),
                        }
                    },
                    else => {
                        // All channels closed, exit the task
                        debug!("All channels closed, exiting dispatcher");
                        break;
                    }
                }
            }
            debug!("Check dispatcher shutdown complete");
        });
...
impl Checks {
    ...
    async fn run_inner(self, context: SourceContext, global_shutdown: ComponentShutdownHandle) -> Result<(), ()> {
        ...
        let tlm = ChecksTelemetry {
            check_requests_dispatched: metrics.register_debug_counter("checkrequests.dispatched"),
            check_instances_started: metrics.register_debug_counter("checkinstances.started"),
        };
    ...
    async fn run(mut self: Box<Self>, mut context: SourceContext) -> Result<(), GenericError> {
        ...
        match self.run_inner(context, global_shutdown).await {
            ...
            Err(e) => {
                ...
                Err(generic_error!("Check source failed: {:?}", e))
            }
        }
    }
}
...
async fn process_listener(
    ...
    loop {
        iteration_count += 1;
        debug!("Listener iteration #{}", iteration_count);
        ...
        select! {
            ...
            _ = tokio::time::sleep(tokio::time::Duration::from_secs(1)) => {
                debug!("Starting check entities update (iteration #{})", iteration_count);
                match listener.update_check_entities().await {
                    Ok(_) => {
                        debug!("Successfully updated check entities (iteration #{})", iteration_count);
                    }
                    Err(e) => {
                        error!("Error updating check entities (iteration #{}): {}", iteration_count, e);
                        // Continue the loop rather than breaking
                    }
                }
                debug!("Completed check entities update (iteration #{})", iteration_count);
            }
            Some(new_entity) = new_entities.recv() => {
                info!("New Check request for {} received.", new_entity.name);
                debug!("Converting check request to runnable request");

                let runnable_check_request: RunnableCheckRequest = new_entity.into();

                debug!("Submitting check request to dispatcher");
                match submit_runnable_check_req.send(runnable_check_request).await {
                    Ok(_) => {
                        trace!("Check request submitted to dispatcher");
                    }
                    Err(e) => {
                        error!("Error submitting check request to dispatcher: {}", e);
                    }
                }
            }
            Some(deleted_entity) = deleted_entities.recv() => {
                info!("Deleting check request for {}", deleted_entity.name);
                match submit_stop_check_req.send(deleted_entity).await {
                    Ok(_) => {
                        debug!("Successfully sent stop check request");
                    }
                    Err(e) => {
                        error!("Error sending stop check request: {}", e);
                    }
                }
            }
        }
    ...
    debug!("Shutting down stream coordinator");
...
impl TryInto<Event> for CheckMetric {
    ...
    fn try_into(self) -> Result<Event, Self::Error> {
        ...
        let tagset: TagSet = TagSet::from_iter(tags);

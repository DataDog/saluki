...
impl Source for DogStatsD {
    async fn run(mut self: Box<Self>, mut context: SourceContext) -> Result<(), GenericError> {
        ...
        // The shrinker just runs until it's told to stop, so let the supervisor drop it where it's parked rather than
        // hand-rolling the same `select!` against a coordinator handle.
        context
            .spawner()
        ...
        select! {
            ...
            result = listener.accept() => match result {
                Ok(stream) => {
                    ...
                    // The child name is the listener type, not anything derived from the peer: every supervised
                    // worker records per-poll telemetry tagged with its process name, so a per-connection name would
                    // be an unbounded label set.
                    ...
                    if let Err(e) = source_context.spawner().spawn_interruptible(task_name, handler).await {
                        // The component's supervisor is guaranteed to be running for the whole of `run`, so this is
                        // terminal rather than something to retry on the next accept.
                        error!(%listen_addr, error = %e, "Failed to spawn stream handler. Stopping listener.");
                        break;
                    }
                }
                Err(e) => {
...
mod supervision {
    ...
    /// Number of connect/send/close cycles the churn harness drives.
    const CHURN_CONNECTIONS: usize = 2_000;

    /// Measures connection churn against a UDS-stream listener.
    ///
    /// Per-connection handlers are supervised children, so each accept costs a channel send plus an acknowledgement
    /// round trip through the component supervisor's single `select!` loop -- serialized against every other child
    /// event that supervisor services. This harness is what makes that cost visible: a UDS-stream listener is the
    /// churn-heavy path, and steady-state throughput profiles don't exercise accept rate at all.
    ///
    /// Ignored by default because it is a measurement, not an assertion -- there is no threshold here that would mean
    /// anything across machines. Run it explicitly, and compare against the same run with the `spawn_interruptible`
    /// call in `process_listener` swapped back to a detached spawn:
    ///
    /// ```text
    /// cargo nextest run -p saluki-components -E 'test(churn_benchmark)' --run-ignored all --no-capture
    /// ```
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    #[ignore = "measurement harness rather than an assertion; run explicitly"]
    async fn churn_benchmark_uds_stream() {
        let socket_dir = tempfile::tempdir().expect("should create a temp dir");
        let socket_path = socket_dir.path().join("dsd.sock");

        let supervisor = TestComponentSupervisor::start("dogstatsd").await;
        let health_registry = HealthRegistry::new();

        // Room to spare on the output, so what's measured is the accept path rather than this test's drain rate.
        // A deep output and a deep buffer pool, so neither this test's drain rate nor buffer-pool contention is what
        // ends up being measured -- only the accept path.
        let mut harness = build_source_with(&supervisor, &health_registry, 4096, 2048).await;
        harness.source.listeners = vec![
            Listener::from_listen_address(ListenAddress::Unix(socket_path.clone()), None)
                .await
                .expect("UDS-stream listener should bind"),
        ];
        ...
        let run = tokio::spawn(async move { source.run(context).await });
        supervisor.wait_for_children(1 + DECODER_WORKERS + 1).await;

        // Drain concurrently, so a full output never becomes the thing being timed.
        let drainer = tokio::spawn(async move {
            let mut seen = 0;
            while seen < CHURN_CONNECTIONS {
                match metrics_rx.recv().await {
                    Some(buffer) => seen += buffer.len(),
                    None => break,
                }
            }
            seen
        ...
        // UDS-stream frames are length-delimited (a `u32` little-endian prefix), not raw newline-terminated text.
        // One fixed context, so this measures the accept path rather than context-resolver churn.
        let payload = b"churn.metric:1|c\n";
        let mut frame = Vec::with_capacity(4 + payload.len());
        frame.extend_from_slice(&(payload.len() as u32).to_le_bytes());
        frame.extend_from_slice(payload);

        let started = std::time::Instant::now();
        let mut connect_latencies = Vec::with_capacity(CHURN_CONNECTIONS);
        ...
        let seen = timeout(Duration::from_secs(60), drainer)
            .await
            .expect("every churned metric should be dispatched")
            .expect("drainer should not panic");

        connect_latencies.sort_unstable();
        let p50 = connect_latencies[connect_latencies.len() / 2];
        let p99 = connect_latencies[connect_latencies.len() * 99 / 100];
        let rate = CHURN_CONNECTIONS as f64 / churn_elapsed.as_secs_f64();

        eprintln!(
            "churn: {CHURN_CONNECTIONS} connections in {churn_elapsed:?} ({rate:.0}/s), \
             connect p50 {p50:?}, p99 {p99:?}, {seen} metrics dispatched"
        );

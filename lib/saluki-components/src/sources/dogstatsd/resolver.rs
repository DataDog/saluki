...
impl ContextResolvers {
    ...
    pub fn new(
        ...
        let cached_contexts_limit = config.source.cached_contexts_limit;
        let cached_tagsets_limit = config.source.cached_tagsets_limit;
        let context_expiry_seconds = Duration::from_secs(config.source.context_expiry_seconds);
        let allow_context_heap_allocations = config.source.allow_context_heap_allocations;

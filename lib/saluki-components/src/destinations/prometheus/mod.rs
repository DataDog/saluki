...
use saluki_common::{collections::FastIndexMap, strings::StringBuilder};
            ...
            select! {
                ...
                maybe_events = context.events().next() => {
                    ...
                    for event in events {
                        ...
                        if !grouped_values.update(&context, &values) {
                            ...
                            let prom_tags = match try_context_to_prom_tags(&context, &mut tags_builder) {
                ...
                Some(payload_resp_tx) = payload_req_rx.recv() => {
                    ...
                    if render_required {
                        ...
                        latest_payload.push_str(payload_builder.string());
                    }
...
fn render_payload(
    ...
    for (metric_name, grouped_values) in metrics {
        ...
        if payload_builder.push_str(subpayload_builder.string()).is_none() {
            debug!(
                metrics_written,
                metrics_total,
...
fn try_context_to_prom_tags(context: &Context, builder: &mut InternedStringBuilder) -> Option<MetaString> {
    match format_tags(context, builder) {
        Some(tags) => Some(tags),
        None => {
            debug!(
...
fn format_tags(context: &Context, builder: &mut InternedStringBuilder) -> Option<MetaString> {
    builder.clear();

...
impl TraceSampler {
    ...
    fn has_analyzed_spans(&self, trace: &Trace) -> bool {
        trace
            .spans()
            .iter()
            .any(|span| span.attributes.get(KEY_ANALYZED_SPANS).and_then(AttributeValue::as_float).is_some())
    }
    ...
    fn run_samplers(&mut self, trace: &mut Trace) -> (bool, i32, &'static str, Option<usize>) {
        ...
        if self.probabilistic_sampler_enabled {
            ...
            if rare {
                ...
                // Run probabilistic sampler - use trace's trace ID
                let root_trace_id = trace.trace_id_low;
...
mod tests {
    ...
    fn create_test_span_with_metrics(span_id: u64, metrics: HashMap<String, f64>) -> DdSpan {
        ...
        create_test_span(span_id, 0).with_metrics(metrics_map)
    }
    ...
    fn create_test_span_with_meta(span_id: u64, meta: HashMap<String, String>) -> DdSpan {
        ...
        create_test_span(span_id, 0).with_meta(meta_map)
    }
    ...
    fn test_single_span_sampling() {
        ...
        let sss_span = create_test_span(1, 0).with_metrics(metrics_map.clone());
    ...
    fn test_analytics_events() {
        ...
        let analyzed_span = create_test_span(1, 0).with_metrics(metrics_map.clone());
        let regular_span = create_test_span(2, 0);
        ...
        let analyzed_span_ids: Vec<u64> = trace
            .spans()
            .iter()
            .filter(|span| span.attributes.get(KEY_ANALYZED_SPANS).and_then(AttributeValue::as_float).is_some())
    ...
    fn test_probabilistic_sampling_with_prob_rate_key() {
        ...
        if keep && decision_maker == DECISION_MAKER_PROBABILISTIC {
            ...
            assert!(root_span.attributes.get(PROB_RATE_KEY).and_then(AttributeValue::as_float).is_some());
            assert_eq!(root_span.attributes.get(PROB_RATE_KEY).and_then(AttributeValue::as_float).unwrap(), 0.75);
            ...
            assert!(modified_root.attributes.get(TAG_DECISION_MAKER).and_then(AttributeValue::as_string).is_some());
            assert_eq!(
                modified_root.attributes.get(TAG_DECISION_MAKER).and_then(AttributeValue::as_string).unwrap(),
    ...
    fn create_top_level_span(span_id: u64) -> DdSpan {
        ...
        create_test_span(span_id, 0).with_metrics(metrics)
    }
    ...
    fn rare_sampler_catches_priority_auto_drop_in_legacy_path() {
        ...
        let span = create_test_span(1, 0).with_metrics(metrics);
        let mut trace = create_test_trace(vec![span]);
    ...
    fn rare_sampler_catches_otlp_no_priority_trace() {
        ...
        let span = create_top_level_span(1).with_meta(meta);
        let mut trace = create_test_trace(vec![span]);
    ...
    fn ets_keeps_trace_with_exception_span_event() {
        ...
        let span = create_test_span(1, 0).with_meta(meta);
        let mut trace = create_test_trace(vec![span]);
    ...
    fn create_otlp_test_span(span_id: u64, error: i32) -> DdSpan {
        ...
        create_test_span(span_id, error).with_meta(meta)
    }
    ...
    fn ets_otlp_user_priority_gets_manual_dm() {
        ...
        let mut metrics = saluki_common::collections::FastHashMap::default();
        metrics.insert(MetaString::from(SAMPLING_PRIORITY_METRIC_KEY), 2.0); // UserKeep
        let span = create_otlp_test_span(1, 0).with_metrics(metrics); // no error
        let mut trace = create_test_trace(vec![span]);

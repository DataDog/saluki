...
use saluki_components::bench::normalize_tag;
...
fn measure_normalize_cpu_time<F, R>(iters: u64, normalize_fn: F) -> Duration
where
    F: Fn(&str) -> R,
{
...
fn bench_normalize_tag(c: &mut Criterion) {
    ...
    group.bench_function("normalize_tag", |b| {
        b.iter_custom(|iters| measure_normalize_cpu_time(iters, normalize_tag));
    });
    group.finish();
}

...
use saluki_components::bench::{normalize_scalar, normalize_simd};
...
fn bench_normalizer<F, R>(normalize_fn: F) -> impl FnMut(&mut Bencher<'_>)
where
    F: Fn(&str) -> R + Copy,
{
    move |b| {
        b.iter(|| {
            for (_, values) in black_box(OTLP_TAG_VALUE_DATASETS) {
                for &value in values.iter() {
                    black_box(normalize_fn(black_box(value)));
                }
            }
        });
    }
...
fn bench_normalize_tag(c: &mut Criterion) {
    let mut group = c.benchmark_group("normalize_tag/realistic_otlp_values");
    ...
    group.bench_function("scalar", bench_normalizer(normalize_scalar::normalize));
    group.bench_function("simd", bench_normalizer(normalize_simd));
    group.finish();
}

criterion_group!(benches, bench_normalize_tag);

...
mod tests {
    ...
    async fn dedicated_runtime_subtree_is_visible_across_the_thread_boundary() {
        ...
        wait_until("the dedicated-runtime subtree is visible", || {
            tree.snapshot()
                .root
                .children
                .iter()
    ...
    async fn snapshot_after_the_run_reports_a_stopped_tree() {
        ...
        // Not running is not the same as never having run: the node keeps the identity of the run that just ended, so
        // that a stopped tree still says what it was. `state` is what distinguishes the two.
        assert!(snapshot.root.process_id.is_some(), "a stopped node says what it ran as");
        assert_eq!(snapshot.root.process_name.as_deref(), Some("test_sup"));
        assert!(snapshot.root.started_at.is_some());
        assert_eq!(snapshot.root.uptime_ms, None, "a node that isn't running has no uptime");
    }

    #[tokio::test]
    async fn an_exited_nested_supervisor_says_what_it_ran_as() {
        // A nested supervisor that stops for good is kept by its parent as a tombstone, and what it ran as is what a
        // postmortem wants from it -- which is what its own workers have always reported in the same situation.
        let mut child_sup = Supervisor::new("child-sup")
            .unwrap()
            .with_auto_shutdown(AutoShutdown::AnySignificant);
        child_sup.add_worker(
            runtime::supervisable(MockWorker::completing("one-shot", Duration::from_millis(10)))
                .temporary()
                .with_significant(true)
                .build(),
        ...
        // Temporary, so the parent does not bring it back once it stops.
        let mut parent_sup = Supervisor::new("parent-sup").unwrap();
        parent_sup.add_worker(ChildSpecification::from(child_sup).with_restart_type(RestartType::Temporary));

        let tree = parent_sup.tree_handle();
        let (tx, handle) = run_supervisor_with_trigger(parent_sup).await;

        // Record the identity while it is still running, so the tombstone has to report the same one rather than
        // merely reporting something.
        let mut running = None;
        wait_until("the nested supervisor is running", || {
            let snapshot = tree.snapshot();
            let nested = find_node(&snapshot.root.children, "child-sup");
            if nested.state == NodeState::Running {
                running = Some((nested.process_id, nested.process_name.clone(), nested.started_at));
                return true;
            }
            false
        })
        .await;
        let (process_id, process_name, started_at) = running.expect("observed running");
        assert!(process_id.is_some(), "a running supervisor has a process");

        wait_until("the nested supervisor has exited", || {
            find_node(&tree.snapshot().root.children, "child-sup").state == NodeState::Exited
        })
        .await;

        let snapshot = tree.snapshot();
        let nested = find_node(&snapshot.root.children, "child-sup");
        assert_eq!(nested.process_id, process_id, "an exited supervisor keeps its identity");
        assert_eq!(nested.process_name, process_name);
        assert_eq!(nested.started_at, started_at);
        assert!(nested.exited_at.is_some(), "a tombstone records when it exited");
        assert_eq!(nested.uptime_ms, None, "a node that isn't running has no uptime");
        assert!(
            nested.resource_group.is_some(),
            "and the group its allocations were attributed to"
        );

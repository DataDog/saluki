...
use crate::{
    runtime::state::{AcquireError, Resource, ResourceLease, ResourceRegistry},
...
impl BuildContext {
    ...
    pub async fn acquire_resource<R: Resource>(
        &self, spec: R::Specification,
    ) -> Result<ResourceLease<R>, AcquireError> {
        self.resource_registry.acquire::<R>(&self.identity(), spec).await
    }
}

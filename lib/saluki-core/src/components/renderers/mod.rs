...
/// Renderer process result.
pub enum ProcessResult {
    /// The renderer processed the event successfully and is ready to process more events.
    Continue,

    /// The renderer cannot process the event without flushing first.
    ///
    /// The caller should flush the renderer and try again to process the event.
    FlushRequired(Event),
}
...
///
/// Incremental renderers represent the essential operations of a renderer: adding events and flushing the resulting
/// payloads. Generally, renderers should not need to concern themselves with the high-level details of being a
/// component within a topology, such as responding to health probes, handling graceful shutdown, and so on. Through
/// separating out the core renderer functionality from the component functionality, we can make it easier to implement
/// various renderer implementations while only maintaining a few common renderer components that actually _drive_ the
/// underlying renderer logic.
#[async_trait]
pub trait IncrementalRenderer {
    /// Process a single event.
    ///
    /// The renderer will process the event, attempting to add it to the current payload. If the renderer is unable to
    ...
    /// # Errors
    ///
    /// If the renderer cannot process the event due to an unrecoverable error, an error is returned.
    async fn process_event(&mut self, event: Event) -> Result<ProcessResult, GenericError>;

    /// Flush the renderer, finalizing all current payloads and sending them to the forwarder.
    ///
    /// # Errors
    ///
    /// If the renderer cannot flush the payloads, an error is returned.

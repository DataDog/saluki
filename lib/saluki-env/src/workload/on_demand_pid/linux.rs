...
impl ResolverImpl {
    /// Creates a new `ResolverImpl` from the given cgroups configuration, along with the worker that drives its
    /// background work.
    ///
    /// The worker expires idle PID mappings and reports interner utilization. Neither happens while it isn't running.
    /// Add it to a supervisor as a transient child: both loops also stop when the resolver they serve is dropped, which
    /// is a clean exit that a permanent child would restart into a loop.
    ///
    /// # Errors
    ///
    ...
    pub fn new(
        ...
        let interner_telemetry = InternerTelemetry {
            interner: interner.clone(),
            telemetry,
            shutdown: telemetry_shutdown.clone(),
        };
        let worker = CompositeWorker::new(
            "on_demand_pid_resolver",
            vec![Box::new(cache_worker), Box::new(interner_telemetry)],
        );
...
/// A worker that reports a [`ResolverImpl`]'s interner utilization.

...
//!
//! [`generate`] writes these into `src/config_registry/generated/`, which `.gitignore` excludes:
//! the files are ordinary modules a reader can open and grep, but they never reach a commit or a
//! review. Each file gets a `use super::*` preamble, and the hand-written `generated/mod.rs`
//! `include!()`s `annotations_index.rs`.
//!
//! ## Ordering
//!
...
/// Write the annotation tables into `generated_dir` as plain modules.
pub fn generate(overlay: &SchemaOverlay, schema_map: &IndexMap<String, FieldInfo>, generated_dir: &Path) {
    std::fs::create_dir_all(generated_dir).unwrap();

    validate_saluki_entries(overlay);

    let preamble = "#[allow(unused_imports)]\nuse super::schema;\n#[allow(unused_imports)]\nuse super::*;\n\n";
    generate_subsystem_files(overlay, schema_map, generated_dir, preamble);
    generate_unsupported_rs(overlay, generated_dir, preamble);
    generate_annotations_index(overlay, generated_dir);
}
...
fn generate_subsystem_files(
    ...
    for (yaml_path, entry) in &overlay.inventory {
        ...
        // Fields not supported by smoke testing
...
fn generate_one_file(
    ...
    write_if_changed(&dir.join(filename), &out);
}
...
fn generate_unsupported_rs(overlay: &SchemaOverlay, dir: &Path, preamble: &str) {
    ...
    write_if_changed(&dir.join("unsupported.rs"), &out);
}
...
fn generate_annotations_index(overlay: &SchemaOverlay, dir: &Path) {
    ...
    write_if_changed(&dir.join("annotations_index.rs"), &out);
}

/// Write `contents` to `path` only when it differs from what is already there.
///
/// These files live under `src/`, where a new modification time costs a rebuild of the crate.
fn write_if_changed(path: &Path, contents: &str) {
    if std::fs::read_to_string(path).is_ok_and(|existing| existing == contents) {
        return;
    }
    std::fs::write(path, contents).unwrap_or_else(|e| panic!("cannot write {}: {}", path.display(), e));
}

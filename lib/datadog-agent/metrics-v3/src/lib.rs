...
//!
//! [`V3Writer`] accumulates metrics one at a time via [`V3Writer::write`], then [`V3Writer::into_columns`]
//! produces the encoded columns. [`V3Writer::finalize`] additionally serializes those columns into a
//! protobuf payload, using a small hand-rolled wire-format encoder rather than an external protobuf
//! library. This crate has no runtime dependencies and works unmodified under `no_std` (plus `alloc`);
...
pub use constants::{
    COLUMN_NAMES, DICT_NAME_STR_FIELD_NUMBER, DICT_ORIGIN_INFO_FIELD_NUMBER, DICT_RESOURCE_LEN_FIELD_NUMBER,
...
pub use writer::{V3EncodedData, V3EncodedMetrics, V3EncoderStats, V3MetricBuilder, V3ValueEncodingStats, V3Writer};

...
\
    ...
    SalukiKey {
        ...
        documentation: Some(concat!(
            ...
            "Metric prefixes for the same tag must not overlap, including duplicate prefixes. ADP rejects overlapping ",
            "rules at startup so each metric and tag matches at most one rule. Rules for different tags may use the ",
            "same prefix. ADP also rejects empty metric prefixes and tag names, and tag names containing `:`. As with ",

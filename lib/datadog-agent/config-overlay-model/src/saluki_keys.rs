...
pub static SALUKI_KEYS: &[SalukiKey] = &[
    ...
    SalukiKey {
        ...
        documentation: Some(concat!(
            ...
            "A metric is forwarded if its name matches this exact list or the endpoint's ",
            "`metric_prefix_allowlist`. If both lists are empty, no metrics are sent to that endpoint. ",
            "Endpoints absent from both policy maps retain their ordinary behavior.\n\n",
            ...
            "pair of policy maps leaves ordinary endpoint routing unchanged.\n\n",
    ...
    SalukiKey {
        yaml_path: "experimental.metrics_endpoint_routing.metric_prefix_allowlist",
        description: "Per-endpoint literal metric-prefix allow lists",
        default: "{}",
        documentation: Some(concat!(
            "### `experimental.metrics_endpoint_routing.metric_prefix_allowlist`\n\n",
            "Allows metric-name families at selected primary or additional endpoints. Defaults to `{}`. ",
            "Endpoint keys follow the same rules as `metric_allowlist`; an unknown endpoint is rejected. ",
            "These experimental settings may change, move, or be removed without backward compatibility.\n\n",
            "Prefixes are case-sensitive literal strings, using the same starts-with semantics as the DogStatsD ",
            "filterlist's prefix mode. There are no wildcards or regular expressions: `billing.` matches ",
            "`billing.latency` but not `billing_other`. A prefix without the dot, `billing`, matches both. ",
            "An empty string prefix matches every metric name; an empty list allows no names.\n\n",
            "A metric matching either an exact name in `metric_allowlist` or a prefix in this map is forwarded. ",
            "This applies to both series and sketches. Endpoints present in either map are filtered; endpoints ",
            "absent from both keep ordinary delivery. If both lists for a selected endpoint are empty, all its ",
            "metrics are dropped. Prefixes also allow future names in the family, ",
            "so choose them carefully to avoid forwarding more traffic than intended.\n\n",
        ...
        additional_yaml_paths: &[],
        used_by: &["TYPED_CONFIG_SYSTEM"],
        test_json: Some(r#"{"https://primary.example.com":["billing."]}"#),
        pipeline_affinity: "PipelineAffinity::CrossCutting",
        filename: "metrics_endpoint_routing.rs",
    },
    // ── data_plane.rs ────────────────────────────────────────────────────────

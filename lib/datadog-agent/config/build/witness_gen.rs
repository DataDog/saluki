...
/// Whether a leaf parameter type (rendered source text) is a `Copy` scalar that can be passed by
/// value rather than cloned out of the borrowed config.
///
/// Deliberately conservative: only the unambiguous built-in scalars. Anything else (including
/// generated enums, whose `Copy`-ness we cannot tell from the type name) keeps its clone.
fn is_copy_scalar(type_text: &str) -> bool {
    matches!(
        type_text.trim(),
        "bool"
            | "char"
...
fn render(resolved: &[ResolvedKey]) -> String {
    ...
    // Trait.
    out.push_str("/// We refer to the `DatadogConfigConsumer` trait as a \"witness\" trait because it forces an\n");
    out.push_str("/// implementor to deal with each and every supported `DatadogConfiguration` field. Auditability\n");
    out.push_str("/// flows from `schema_overlay.yaml`, where we declare a Datadog schema item as supported. The\n");
    out.push_str("/// `DatadogConfiguration` type is generated from there to deserialize our supported slice of\n");
    out.push_str("/// Agent configuration. Then `drive()` is called to convert it to ADP configuration through a\n");
    out.push_str(
        "/// manual implementation of the generated witness trait. So when you add a supported config, this\n",
    );
    out.push_str("/// trait automatically gets a new method, and you must implement that method at the translation\n");
    out.push_str("/// site.\n");
    ...
    // Emit the consume calls. A `Copy` scalar leaf is passed by value; only non-`Copy` leaves
    // (String, Vec, HashMap, serde_json::Value, ...) need a clone out of the borrowed config.
    for key in resolved {
        let owner = match key.sections.last() {
            Some(step) => step.binding.clone(),
            None => "config".to_string(),
        };
        let access = if is_copy_scalar(&key.type_text) {
            format!("{}.{}", owner, key.leaf_field)
        } else {
            format!("{}.{}.clone()", owner, key.leaf_field)
        };

...
use crate::translate_error::{TranslateError, TranslateErrors};
...
pub trait DatadogConfigWitness {
    ...
    /// Drains every translation error recorded while consuming.
...
pub fn drive(config: &DatadogConfiguration, consumer: &mut impl DatadogConfigWitness) -> Result<(), TranslateErrors> {
    ...
    consumer.consume_dogstatsd_pipe_name(config.dogstatsd_pipe_name.clone());
    ...
    consumer.consume_use_v3_api_series_endpoints(config.use_v3_api.series.endpoints.clone());
    ...
    consumer.consume_vector_metrics_use_v3_api_series(config.vector.metrics.use_v3_api.series.clone());
    consumer.consume_vsock_addr(config.vsock_addr.clone());

    let errors = consumer.translate_errors();
    if errors.is_empty() {
        Ok(())
    } else {
        Err(TranslateErrors::new(errors))
    }
}

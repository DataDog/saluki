...
impl DatadogRemapper {
    ...
    pub fn new() -> Self {
        ...
        // TODO: `std::env::vars()` ordering is undefined on Linux (env is case-sensitive), so if
        // both `HTTP_PROXY` and `http_proxy` are set the result is indeterminate -- whichever
        // iterates first wins via the `!values.contains_key(to)` guard. Fix by iterating
        // precedence-ordered candidates instead of all vars.

...
//!
//! These make nested YAML keys and underscore-joined environment names address the same flat
//! configuration keys, so callers using the by-key view observe the same precedence as callers
//! using the typed configuration. They live here so the configuration system does not depend on
//! the component crate for this support.
//
// TODO: remove when every consumer reads the typed nested `DatadogConfiguration`; the typed path
// gets the same environment reachability from the generated tables.
...
///
/// Add this provider to a `ConfigurationLoader` *after* file-based providers and *before*
...
/// `ConfigurationLoader::with_key_aliases` instead—that's handled at file-load time.

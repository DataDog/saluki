...
/// The value shape of a supported key, as declared in the schema. Only the distinction that changes
/// how a flat env value is materialized is retained.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum EnvValueType {
    /// A single value. figment has already coerced booleans and numbers; strings (including the
    /// custom string-parsed types such as durations and listen addresses) pass through verbatim.
    Scalar,
    /// A list of strings. figment leaves an env var as one string, so it is split on whitespace to
    /// match the Agent's space-separated list convention.
    StringList,
}

/// One supported Datadog key: the flat env forms that reach it, its nested path, and its value shape.
...
pub struct EnvOverlayKey {
    ...
    /// The value shape, controlling how the flat value is materialized.
...
pub fn apply_env_overlay(merged: &mut Value, mode: EnvOverlayMode) {
    ...
    for key in ENV_OVERLAY_KEYS {
        ...
        let value = match key.value_type {
            EnvValueType::StringList => to_string_list(flat),
            EnvValueType::Scalar => flat.clone(),
        };
...
/// Materializes a `StringList` value: a whitespace-separated env string becomes an array; anything
/// already structured is taken as-is.
fn to_string_list(value: &Value) -> Value {
    match value {
        Value::String(s) => Value::Array(s.split_whitespace().map(|t| Value::String(t.to_string())).collect()),
        other => other.clone(),
    }
}
...
mod tests {
    ...
    #[test]
    fn string_list_env_value_is_split() {

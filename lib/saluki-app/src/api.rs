...
pub struct APIBuilder<'a> {
    ...
    reflection_builder: tonic_reflection::server::Builder<'a>,
...
impl<'a> APIBuilder<'a> {
    ...
    pub fn new() -> Self {
        Self {
            ...
            tls_config: None,
            reflection_builder: tonic_reflection::server::Builder::configure(),
            session_id: None,
        }
    }
    ...
    {
        // Use tracing::info! instead of info! macro directly
        debug!("Adding gRPC service: {}", <S as NamedService>::NAME);
        self.grpc_router.add_service(svc.clone());
        self.reflection_builder = self.reflection_builder.with_service_name(<S as NamedService>::NAME);
        self
    }
    ...
    pub async fn serve<F>(mut self, listen_address: ListenAddress, shutdown: F) -> Result<(), GenericError>
    where

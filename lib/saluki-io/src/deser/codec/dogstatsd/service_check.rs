...
use super::{helpers::*, message::*, DogstatsdCodecConfiguration, WellKnownTags};
        ...
        while let Some((chunk, tail)) = split_at_delimiter(remaining, b'|') {
            ...
            match &chunk[..2] {
                ...
                _ if chunk.starts_with(TAGS_PREFIX) => {
                    let (_, tags) = all_consuming(preceded(tag(TAGS_PREFIX), metric_tags(config))).parse(chunk)?;
...
mod tests {
    ...
    #[test]
    fn service_check_well_known_tags() {
        // Just ensures that we're populating well-known tags.
        let raw = "_sc|testsvc|1|#dd.internal.card:low";

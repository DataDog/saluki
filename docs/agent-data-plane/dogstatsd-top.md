...
Offline mode reads only the supplied file and does not contact ADP. `top` requires the `--path` value and does not fall back to a file in the current working directory.
...
- **Connection failure**: Verify that the CLI can reach the configured privileged API endpoint.

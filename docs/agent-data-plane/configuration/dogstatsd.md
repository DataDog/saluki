...
| `dogstatsd_experimental_http.enabled`                             | Enable experimental HTTP/H2C DSD listener          | Experimental HTTP/protobuf ingestion path for client-side aggregated DogStatsD metrics; not implemented by ADP. Re-evaluate if this transport graduates from experimental upstream, or if a customer or internal ADP user starts relying on it.                           |
| `dogstatsd_experimental_http.listen_address`                      | Bind address for experimental HTTP DSD listener    | Experimental HTTP/protobuf ingestion path for client-side aggregated DogStatsD metrics; not implemented by ADP. Re-evaluate if this transport graduates from experimental upstream, or if a customer or internal ADP user starts relying on it.                           |

...
| `started_at`, `uptime_ms` | When the node's most recent process started, and how long it has been running. |
| `restart_count` | Restarts since the node was created, including those caused by its supervisor restarting. |
...
A node that has run keeps `process_id`, `process_name`, and `started_at` after it stops, so a stopped process still
says what it ran as. Read `state` to tell whether that process is the current one, and `uptime_ms`, which is present
only while a node is running. These three fields are absent only for a node that has never run.

...
impl RemoteAgentAutodiscoveryProvider {
    /// Creates a new `RemoteAgentAutodiscoveryProvider` from the autodiscovery attachment's client,
    /// along with a [`Supervisor`] that drives the collection and broadcasting of autodiscovery events.
    ///
    /// # Errors
    ///

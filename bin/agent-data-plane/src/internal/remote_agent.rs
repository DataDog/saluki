...
/// Remote-agent services backed by a typed Datadog Agent connection.
...
impl RemoteAgentBootstrap {
    /// Creates remote-agent service state from a typed Datadog Agent connection.
    pub async fn from_connection(connection: DatadogAgentConnection) -> Self {
        Self {
            connection,
...
impl StatusProvider for RemoteAgentImpl {
    async fn get_status_details(
        ...
        self.session_id_middleware(async || {

...
const SENSITIVE_PUBLISHER_DETAIL: &str = "sensitive-publisher-detail";
...
#[tokio::test]
async fn router_post_returns_only_the_json_path_to_a_complete_decodable_artifact() {
    ...
    let handler = DogStatsDContextDumpAPIHandler::new(vec![handle], run_directory.path());
...
async fn no_handles_and_closed_owner_return_service_unavailable() {
    ...
    let response = send(&no_handles, context_dump_post()).await;
    ...
    let response = send(&closed_owner, context_dump_post()).await;
    assert_eq!(response.status(), StatusCode::SERVICE_UNAVAILABLE);
    assert_eq!(
        response_body(response).await,
        "DogStatsD context snapshot is unavailable; retry after the aggregate owner is running."
...
async fn empty_and_non_directory_run_paths_reach_publication_and_return_safe_internal_errors() {
    ...
    for run_path in [PathBuf::new(), non_directory] {
        let (handle, mut responder) = aggregate_context_snapshot_channel_for_test();
        let handler = DogStatsDContextDumpAPIHandler::new(vec![handle], run_path);
        ...
        assert!(!body.contains(SENSITIVE_PUBLISHER_DETAIL));
        owner.await.unwrap().unwrap();
    }
    assert_eq!(
...
async fn requests_serialize_fixed_path_publisher_executions() {
    ...
    let first = tokio::spawn(async move { send(&first_handler, context_dump_post()).await });
    ...
    let second = tokio::spawn(async move { send(&second_handler, context_dump_post()).await });
...
async fn canceling_after_spawn_blocking_starts_still_completes_atomic_publication() {
    ...
    let request = tokio::spawn(async move { send(&request_handler, context_dump_post()).await });
    tokio::task::spawn_blocking(move || started_rx.recv_timeout(Duration::from_secs(1)))
        .await
        .unwrap()
...
fn test_handler(
    ...
    DogStatsDContextDumpAPIHandler::new_for_test(handles, run_path, Duration::from_millis(100), publisher)
}

fn context_dump_post() -> Request<Empty<Bytes>> {
    Request::builder().method("POST").uri(ROUTE).body(Empty::new()).unwrap()
}
...
impl ContextDumpPublisher for FailingPublisher {
    fn publish(
        ...
        Err(generic_error!(
            "injected publisher failure with {SENSITIVE_PUBLISHER_DETAIL}"
        ))
    }
}

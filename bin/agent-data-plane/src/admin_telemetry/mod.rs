...
use saluki_core::{observability::metrics::MetricsReceiver, task::spawn_traced};
...
async fn run_telemetry_collector(state: Arc<TelemetryState>) {
    let mut metrics_rx = MetricsReceiver::register();

    loop {
        select! {
            metrics = metrics_rx.next() => {
                ...
                for metric in metrics.iter() {
                    if let Some(processed_metric) = metric.try_as_metric().and_then(process_metric) {
                        ...
                        snapshot.metrics.insert(processed_metric.name.clone().into_owned(), processed_metric.value);
                    }
                }
...
fn build_telemetry_server(
    state: Arc<TelemetryState>,
) -> CorsGrpcWeb<TelemetryServiceServer<TelemetryServerImpl>> {

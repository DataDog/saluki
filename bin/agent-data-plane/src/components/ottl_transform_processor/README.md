...
- `attributes` (span-level) is **read-write**: `set` can create, overwrite, or remove attributes. `resource.attributes` (trace resource tags) is **read-only**: it can be used in `where` conditions but can't be written to.

...
pub async fn configure_and_spawn_api_endpoints(
    ...
    if !in_standalone_mode {
        let local_secure_api_addr = secure_api_listen_address
    ...
    tokio::spawn(async move {
        info!("Serving privileged API on {}.", secure_api_listen_address);

        if let Err(e) = privileged_api.serve(secure_api_listen_address, pending()).await {

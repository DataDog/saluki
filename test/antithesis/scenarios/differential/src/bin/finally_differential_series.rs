//! Antithesis `finally_` check: every shared context aggregates the same once load has drained.
//!
//! Sleeps one flush budget so the last flushes land, then runs the same comparison.

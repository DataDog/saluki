...
SDK `assert_always!` per W property. See the crate `README.md` at
`test/antithesis/intake/README.md` for the W1-W22 catalog, the numbering, and the
double-sourced Agent and intake references.
...
- Envelope W1-W3 read raw headers. Content-Type, Content-Encoding, API key.
  ...
  cross-cut contract to assert. A `datadog-agent/` check fires on every payload.
...
an unnamed host resource from a wrong name. An earlier run fired W17 on most
payloads but the detail did not carry the resolved value, so the cause was
undetermined. The next run will show it.
...
The predicates read the `rust-protobuf` types in `datadog_protos`. The
agent-payload proto and the field semantics are unchanged. Access reads
`series.metric()`, `r.type_()`, `series.type_.enum_value()`, and `MessageField`
for `metadata` and `origin`.

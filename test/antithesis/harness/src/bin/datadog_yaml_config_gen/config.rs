...
/// supplied by the environment; the rest are drawn.
///
/// Numeric fields are drawn with [`Probe`]: usually a typical value (so ADP
...
impl DogStatsdConfig {
    /// Draw the `DogStatsD` options from `rng`, taking the socket from the
    /// environment.
    fn gen<R: Rng + ?Sized>(rng: &mut R, dogstatsd_socket: &Path) -> Self {
        Self {
...
/// drawn per branch. The static flags are appended by [`Self::to_yaml`], not
/// fields here.
#[derive(Debug, Serialize)]
pub(crate) struct DatadogConfig {
    ...
    dd_url: String,
    /// Agent log verbosity. Drawn; restricted to quiet levels (see [`LogLevel`]).
    log_level: LogLevel,
...
impl DatadogConfig {
    ...
    /// rest are drawn from `rng`. With an Antithesis-backed rng, each call after
    /// the snapshot yields an independent draw per replay branch.
    pub(crate) fn gen<R: Rng + ?Sized>(
        ...
        Self {
            ...
            dogstatsd: DogStatsdConfig::gen(rng, dogstatsd_socket),
        }
    }
...
mod tests {
    ...
    #[test]
    fn renders_expected_yaml_for_fixed_seed() {
        let mut rng = SmallRng::seed_from_u64(0);
        let config = DatadogConfig::gen(
            &mut rng,
            "antithesis-adp",
            "antithesis-test-api-key",
            "http://intake:2049",
            Path::new("/var/run/datadog/dsd.socket"),
        );
        let expected = "\
hostname: antithesis-adp
api_key: antithesis-test-api-key
dd_url: http://intake:2049
log_level: warn
dogstatsd_socket: /var/run/datadog/dsd.socket
dogstatsd_buffer_size: 672
dogstatsd_so_rcvbuf: 65535
dogstatsd_packet_buffer_size: 9223372036854775807
dogstatsd_packet_buffer_flush_timeout: 82ms
dogstatsd_queue_size: 126
dogstatsd_pipeline_count: 1
dogstatsd_workers_count: 3673
dogstatsd_expiry_seconds: 32766
dogstatsd_context_expiry_seconds: 219
dogstatsd_string_interner_size: 48
dogstatsd_mapper_cache_size: 469
dogstatsd_no_aggregation_pipeline_batch_size: 54
dogstatsd_tag_cardinality: orchestrator
dogstatsd_non_local_traffic: true
dogstatsd_origin_detection: false
dogstatsd_origin_detection_client: false
dogstatsd_origin_optout_enabled: false
...
cloud_provider_metadata: []
";
        assert_eq!(config.to_yaml().expect("render datadog.yaml"), expected);
    }
}
